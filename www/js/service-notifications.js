...
var myApp = new Framework7({
	...
	preprocess:function(content,url){
        url_arr=url.split('?');
        historyInfo.contents[url_arr[0]]=content;
        historyInfo.urls[url_arr[0]]=url;
        orgContent=content;
        curTemplateUrl=url;
        var filter_dataArr=new Array();
        filter_dataArr=content.split("<!--header-->");
        if(filter_dataArr.length>1)
            content=filter_dataArr[1];
        return content;
    }
});
...
myApp.onPageInit('dashboard', function (page) {
	...
	var calendarInline = myApp.calendar({
        ...
        toolbarTemplate:
            '<div class="toolbar calendar-custom-toolbar">' +
                '<div class="toolbar-inner">' +
                    '<div class="left">' +
                        '<a href="#" class="link icon-only"><i class="fa fa-angle-double-left"></i></a>' +
                    '</div>' +
                    '<div class="center"></div>' +
                    '<div class="right">' +
                        '<a href="#" class="link icon-only"><i class="fa fa-angle-double-right"></i></a>' +
                    '</div>' +
                '</div>' +
            '</div>',
        onOpen: function (p) {
		...
		onDayClick: function(p, dayContainer, year, month, day){
			...
			mainView.router.reloadPage("services-list.html");
...
$$(document).on('pageInit', function (e) {
    ...
    curQueryString=page.query;

    //alert(page.url);
    //alert("Main View Url::"+mainView.url);
    url_arr=mainView.url.split('?');

    //alert(page.name);
    //alert(orgContent);
    content=orgContent;
    content=historyInfo.contents[url_arr[0]];
    curTemplateUrl=historyInfo.urls[url_arr[0]];
    curQueryString={};
        var filter_dataArr=new Array();
        filter_dataArr=content.split("<!--header-->");
       
        if(url_arr.length>1)
            curQueryString=QueryStringToJSON("?"+url_arr[1]);
        if(filter_dataArr.length>1)
            processTemplateData(content,url_arr[0]);
