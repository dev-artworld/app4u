        ...
        if(data.data == "subcategories"){
          ...
          category_apps_by_id(category_id);
	        ...
	        for(var i=0;i<data.subcategories.length;i++){
	            apps_html += '<a id="subcatmenu" onclick="category_apps_by_id('+data.subcategories[i].id+')" class="tab-link" href="#'+data.subcategories[i].slug+'">'+data.subcategories[i].name+'</a>'; 
	        }

	        jQuery(".sub-tabber").html(apps_html);
			var selector = "a.tab-link";
            ...
            apps_html += '<div class="app-title" onclick="get_app_single_view('+data.apps[i].id+')">'+data.apps[i].post_title+'</div>';
            apps_html += '<div class="rating-price-row">'
			      ...
			      apps_html += '<div class="price-col">'+data.apps[i].price+'</div>';
				    ...
				    apps_html += '</div>';
				    apps_html += '<div class="download-btn-row"><a onclick="get_app_single_view('+data.apps[i].id+')" href="javascript:void(0)">Download <i class="fa fa-chevron-down" aria-hidden="true"></i></a></div>';
				    apps_html += '</li>';
    ...
    success:function(res){
        ...
        for(var i=0;i<data.apps.length;i++){
            ...
            apps_html += '<li>';   
               ...
               apps_html += '<div class="app-title"onclick="get_app_single_view('+data.apps[i].post_id+')">'+data.apps[i].post_title+'</div>';
               apps_html += '<div class="rating-price-row">'
               apps_html += '<div class="rating-col"><img data-rating="'+review+'" src="img/stars-rating-img.png" alt=""/></div>';
               apps_html += '<div class="price-col">'+data.apps[i].price+'</div>';
               apps_html += '<div class="clear"></div>';
               apps_html += '</div>';
               apps_html += '<div class="download-btn-row"><a onclick="get_app_single_view('+data.apps[i].post_id+')" href="javascript:void(0)">Download <i class="fa fa-chevron-down" aria-hidden="true"></i></a></div>';
             apps_html += '</li>';
              }
...
{
       jQuery.ajax({
       url:"http://apps4u.myworkspacefiles.com/?source=app&action=latest_apps",
       type:"Get",
       async:true,
       beforeSend:function(){
       myApp.showIndicator();
     ...
     var data =JSON.parse(res);
        ...
        jQuery(".cat-apps-wrapper ul#latest_six").html(apps_html);
        jQuery('.link.open-picker').click(function(){ 
        var video_url = jQuery(this).attr("src"); 
        jQuery("#video_link").attr("src",video_url) });
      ...
      success:function(res){
          ...
          if(data.apps.length > 0){
            for (var i=0;i<data.apps.length;i++) {
                ...
                Related_html +=     '<a onclick="get_related_app_view('+data.apps[i].id+')" href="javascript:void(0)" class="download-btn">Download</a>';
                Related_html +=  '</li>';
            }
...
function submit_app_review(){
	...
	if(review_name == ""){
		myApp.alert("Warning! Please Enter Your Review Name.");
		return false;
	}
	var review_email = jQuery("input[name='review_email']").val();
	if(review_email == ""){
		myApp.alert("Warning! Please Enter Your Review Email.");
		return false;
	}
	var review_website = jQuery("input[name='review_website']").val();
	if(review_website == ""){
		myApp.alert("Warning! Please Enter Your Review Website.");
		return false;
	}
	var review_title = jQuery("input[name='review_title']").val();
	if(review_title == ""){
		myApp.alert("Warning! Please Enter Your Review Title.");
		return false;
	}
	var rating = jQuery("input[name='star']:checked").val();
	if(rating == ""){
		myApp.alert("Warning! Please Enter Your Review Rating.");
		return false;
	}
	var review_content = jQuery("textarea[name='review_content']").val(); 
	if(review_content == ""){
		myApp.alert("Warning! Please Enter Your Review Content.");
		return false;
	}
    ...
    if (confirm("Are you sure!")) {
      ...
      myApp.alert("Thank you! You have successfully submitted review for this Application!")
...
function shareurl(url) {
  ...
  var link = jQuery(url).attr("data-link");
  cordova.InAppBrowser.open(link);
}

function stopVideo(){

   jQuery("#video_link").attr('src','');
}
